...
- repo: `scripts/verify-revere-menubar.sh` rebuilds, signs, installs, launches, and click-through verifies the native menu-bar app via System Events; use `--no-build` after granting Screen Recording permission so the verifier preserves the installed app identity that macOS approved. Once Screen Recording is granted, the verifier also starts/stops Visual Watch and checks sample updates; once Camera is already granted, it runs screen+face and face-only recording tests without triggering a new prompt.
